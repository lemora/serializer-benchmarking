...
public class ProtostuffSerSizer {

    private static final String BENCHMARKER = "ProtostuffSerSizer";
    private static final String SERTYPE = "protostuff";
    ...
    public static void evaluateSerSizes() throws FileNotFoundException, UnsupportedEncodingException {
        ...
        resultList.add( new BenchmarkResult(SerializerProtostuff.ser_Msg(new Msg()).length, 0, SERTYPE + "_msg00_ser") );
        resultList.add( new BenchmarkResult(SerializerProtostuff.ser_MsgRw01(new Msg_rw01()).length, 0, SERTYPE + "_msg01_ser") );
        resultList.add( new BenchmarkResult(SerializerProtostuff.ser_MsgRw02(new Msg_rw02()).length, 0, SERTYPE + "_msg02_ser") );
        resultList.add( new BenchmarkResult(SerializerProtostuff.ser_MsgRw03(new Msg_rw03()).length, 0, SERTYPE + "_msg03_ser") );
        resultList.add( new BenchmarkResult(SerializerProtostuff.ser_MsgRw04(Msg_rw04.createTestInstance()).length, 0, SERTYPE + "_msg04_ser") );
        resultList.add( new BenchmarkResult(SerializerProtostuff.ser_MsgRw05(new Msg_rw05()).length, 0, SERTYPE + "_msg05_ser") );
        ...
        for(int i = 0; i < containerVals.length; i++) {
            ...
            switch (containerType) {
                case STRING_LIST:
                    ...
                    resultList.add( new BenchmarkResult(SerializerProtostuff.ser_StringList(sl).length,  "sersize_stringlist_" + valCount, valCount) );
                    break;
                case INT_LIST:
                    IntList il = new IntList(Helper.getIntListFromCSV(valCount));
                    resultList.add( new BenchmarkResult(SerializerProtostuff.ser_IntList(il).length,  "sersize_intlist_" + valCount, valCount) );
                    break;
                case DOUBLE_LIST:
                    DoubleList dl = new DoubleList(Helper.getDoubleListFromCSV(valCount));
                    resultList.add( new BenchmarkResult(SerializerProtostuff.ser_DoubleList(dl).length, "sersize_doublelist_" + valCount, valCount) );
                    break;
                case ANY:
                    break;
            }
    ...
    public static void evaluateSerSizesDcachey() throws IOException {
        List<BenchmarkResult> resultList = new LinkedList<>();
        resultList.add( new BenchmarkResult(SerializerProtostuff.ser_PMPUM(PoolManagerPoolUpMessage.createTestInstance()).length, 0, SERTYPE + "_pmpum_ser") );
